...
public class RockAreaScript : NetworkBehaviour {
    ...
    public void Start() {
        ...
        for (int line = 0; line < 4; ++line) {
            ...
            for (int column = 0; column < 10; ++column) {
                // Create rocks
                GameObject go = Instantiate(rock, transform.position + new Vector3(direction * line * 0.5f, (column * 0.5f) + loc, 0f), Quaternion.identity);
                NetworkServer.Spawn(go);
                go.transform.parent = this.transform;

                this.rocks[line][column] = go.GetComponent<RockScript>();
